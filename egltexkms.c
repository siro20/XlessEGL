...
int main(int argc, char *argv[])
{
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, rsize, rsize, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL);  
	...
	data = malloc(rsize * rsize * 4);
	for(j=0; j<rsize; j++)
		for(i=0; i<rsize; i++)
		{
			data[((j*(int)rsize)+i)*4] = i & 0x10 ? 0xff : 0;
			data[((j*(int)rsize)+i)*4+1] = j & 0x10 ? 0xff : 0;
			data[((j*(int)rsize)+i)*4+2] = (i+j) & 0x10 ? 0xff : 0;
			data[((j*(int)rsize)+i)*4+3] = 0xff;
		}
	glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, rsize, rsize, GL_RGBA, GL_UNSIGNED_BYTE, data);
	free(data);
	glBindTexture(GL_TEXTURE_2D, 0);
